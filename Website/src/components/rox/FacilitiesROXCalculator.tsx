...
const inputFocusStyle = "focus:border-[#5B4499] focus:shadow-[0_0_0_3px_rgba(91,68,153,0.15)]";
...
function CategoryCard({
  ...
  const barColor = score !== null ? getTierColor(score) : "rgba(184,160,216,0.12)";
  ...
  return (
    <div
      style={{
        ...
        borderRadius: "16px",
        padding: "32px",
        boxShadow: "0 1px 3px rgba(107,63,160,0.04)",
      }}
    >
      <span
        style={{
      ...
      <div style={{ marginTop: "24px", paddingTop: "16px", borderTop: "1px solid rgba(6,19,65,0.06)" }}>
        ...
        <div style={{ height: "4px", borderRadius: "2px", background: "rgba(107,63,160,0.08)", overflow: "hidden" }}>
          <div
            style={{
              height: "100%",
              width: barWidth,
...
interface FormData {
  ...
  jobTitle: string;
  phone: string;
  facilityToursPerQuarter: string;
}
...
const tierInterpretations: Record<string, { headline: string; body: string }> = {
  "Critical Gap": {
    headline: "Your facility visits are costing more than they are producing.",
    body: "Your facility is hosting visitors, but very little of that activity is translating into pipeline. Interactions are not being captured, engagement is shallow, and follow-up is delayed. Without change, every tour is a cost, not an investment.",
  },
  "Needs Optimization": {
    headline: "You are capturing some value, but leaving ROX on the table.",
    body: "You are getting results from your facility tours, but there are clear inefficiencies. Engagement quality or follow-up speed is likely dragging your score down. High-intent visitors are slipping away in the gap between the tour and the inbox. Closing those gaps compounds quickly.",
  },
  "High ROX": {
    headline: "Your facility is performing above average. Now prove it with real data.",
    body: "You are outperforming most facility organizations. But without the right platform, you are relying on memory and manual processes. Momentify helps you capture, measure, and scale what is already working.",
  },
  "Elite ROX": {
    headline: "Highly optimized across every category. Keep it that way.",
    body: "Your facility team is operating at the top tier. Every visit is captured, engagement is deep, follow-up is fast, and tours progress to pipeline. Momentify helps you maintain this edge, scale across locations, and prove the ROX to leadership.",
  },
};
...
const impactLines: Record<string, { headline: string; body: string }> = {
  "Visitor Capture Rate": {
    headline: "Momentify captures every facility interaction, not just the ones guides remember.",
    body: "Zone tracking, check-in flows, and engagement tagging give you a true capture rate across every touchpoint.",
  },
  "Engagement Quality": {
    headline: "See exactly which zones and content drive the deepest engagement.",
    body: "Content selections, session attendance, and activation notes are all tracked in real time.",
  },
  "Follow-Up Speed": {
    headline: "CRM entries happen before the visitor leaves the building.",
    body: "Smart Columns score and assign leads automatically. Same-day follow-up becomes the default.",
  },
  "Pipeline Attribution": {
    headline: "Tie facility tours to real pipeline movement, not just visitor logs.",
    body: "Engagement-based scoring surfaces your highest-value visitor interactions so your team focuses where it counts.",
  },
};
...
function getFollowUpContext(days: number): { text: string; color: string } {
  if (days === 0) return { text: "Same-day entry. Elite performance.", color: "#0CF4DF" };
  if (days <= 2) return { text: "Strong. Most facility teams average 3-5 days.", color: "#5FD9C2" };
  if (days <= 5) return { text: "Room to improve. Visitor intent fades fast after a tour.", color: "#F2B33D" };
  return { text: "High risk of lost context. Speed is the biggest gap.", color: "#E5484D" };
}

/* ── Main component ──────────────────────────────────── */

export default function FacilitiesROXCalculator() {
  ...
  const categoryNames = ["Visitor Capture Rate", "Engagement Quality", "Follow-Up Speed", "Pipeline Attribution"];
  ...
  function ResultsScreen() {
    ...
    const handleDownloadPDF = async () => {
      ...
      clone.style.background = "linear-gradient(135deg, #0D0820 0%, #3A2073 55%, #5B4499 100%)";
      ...
      const canvas = await html2canvas(clone, { scale: 2, backgroundColor: "#0D0820", useCORS: true });
      ...
      // Header
      pdf.setFillColor(13, 8, 32);
      pdf.rect(0, 0, pdfWidth, 40, "F");
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(11);
      pdf.setTextColor(255, 255, 255);
      pdf.text("MOMENTIFY  |  Facilities ROX\u2122 Scorecard", 24, 26);
      pdf.setFontSize(9);
      pdf.setTextColor(184, 160, 216);
      pdf.text("momentify.com/rox/facilities", pdfWidth - 24, 26, { align: "right" });
      pdf.addImage(imgData, "PNG", 0, 40, pdfWidth, pdfHeight);
      pdf.save(`Momentify-Facilities-ROX-Score-${displayScore}.pdf`);
    };

    const handleEmailResults = () => {
      const subject = encodeURIComponent(`My Facilities ROX Score: ${displayScore} (${tierName})`);
      const body = encodeURIComponent(
        `My Facilities ROX Score: ${displayScore} / 100\nTier: ${tierName}\n\n` +
    ...
    return (
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "48px 0 64px" }}>
        ...
        <div className="btn-row-center justify-between mb-8">
          <div>
            ...
            <h2 style={{ fontFamily: "var(--font-inter)", fontWeight: 600, fontSize: "clamp(32px, 3.5vw, 46px)", lineHeight: 1.15, letterSpacing: "-0.02em" }}>
              <span style={{ color: "#FFFFFF" }}>Facilities</span><br />
              <span style={{ background: "linear-gradient(135deg, #5B4499, #B8A0D8)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>ROX&#8482; Scorecard</span>
            </h2>
          </div>
        ...
        <div id="rox-results">
        {/* Score display */}
        <div className="mb-8" style={{ background: "rgba(255,255,255,0.9)", borderRadius: "20px", padding: "48px 32px", border: "1px solid rgba(107,63,160,0.1)", boxShadow: "0 1px 3px rgba(107,63,160,0.04)" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          {categoryNames.map((name, i) => {
            ...
            const interpretations: Record<string, string> = {
              "Visitor Capture Rate": s !== null && s >= 70 ? "Strong capture rate relative to total facility tours." : "Too many tours are happening without documented interactions.",
              "Engagement Quality": s !== null && s >= 70 ? "Visitors are engaging meaningfully with your content and demos." : "Interactions are shallow. Visitors are not engaging with content.",
              "Follow-Up Speed": s !== null && s >= 70 ? "Your team is entering data quickly after tours." : "Visitors are going cold before your team logs the tour.",
              "Pipeline Attribution": s !== null && s >= 70 ? "Strong tour-to-pipeline progression rate." : "Documented tours are not advancing to the next stage.",
            };
        ...
        {lowestCategories.length > 0 && (
          <div className="mb-12" style={{ background: "rgba(255,255,255,0.9)", borderRadius: "16px", padding: "32px", border: "1px solid rgba(107,63,160,0.1)", boxShadow: "0 1px 3px rgba(107,63,160,0.04)" }}>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {lowestCategories.map((cat) => {
                ...
                return (
                  <div key={cat.name}>
                    <p style={{ fontFamily: "var(--font-inter)", fontWeight: 600, fontSize: "11px", color: "#6B3FA0", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "8px" }}>
                      {cat.name}
                    </p>
          ...
          <AnimatePresence mode="wait">
            {!formSubmitted ? (
              <motion.div key="form" initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }}>
                <p style={{ fontFamily: "var(--font-inter)", fontWeight: 600, fontSize: "11px", color: "#6B3FA0", letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: "12px" }}>
                ...
                <form onSubmit={handleFormSubmit}>
                    ...
                    {[
                      ...
                      <div key={field.key}>
                        <label style={{ fontFamily: "var(--font-inter)", fontWeight: 500, fontSize: "13px", color: "rgba(6,19,65,0.6)", marginBottom: "6px", display: "block" }}>
                          {field.label}{field.required && <span style={{ color: "#6B3FA0" }}> *</span>}
                        </label>
                        <input
                          type={field.type}
                  ...
                  <div className="mb-6">
                    <label style={{ fontFamily: "var(--font-inter)", fontWeight: 500, fontSize: "13px", color: "rgba(6,19,65,0.6)", marginBottom: "6px", display: "block" }}>
                      How many facility tours does your team give per quarter?
                    </label>
                    <select
                      value={formData.facilityToursPerQuarter}
                      onChange={(e) => setFormData((prev) => ({ ...prev, facilityToursPerQuarter: e.target.value }))}
                      className={inputFocusStyle}
                  ...
                  <button
                    ...
                    style={{
                      background: "linear-gradient(135deg, #6B3FA0, #5B4499)",
                      color: "#FFFFFF",
                      fontFamily: "var(--font-inter)",
                      fontWeight: 700,
                      fontSize: "16px",
              ...
              <motion.div key="confirmation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} className="text-center py-8">
                ...
                >
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#6B3FA0" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <p style={{ fontFamily: "var(--font-inter)", fontWeight: 400, fontSize: "15px", color: "rgba(6,19,65,0.55)", lineHeight: 1.6, maxWidth: "480px", margin: "0 auto" }}>
                  We will be in touch shortly to confirm your consultation time. In the meantime, explore how Momentify works for facility teams.
                </p>
                <a
    ...
    <>
      ...
      >
        ...
        <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
          ...
          {/* Ambient glow orbs */}
          <div style={{ position: "absolute", top: "20%", right: "-10%", width: "600px", height: "600px", borderRadius: "50%", background: "#B8A0D8", opacity: 0.07, filter: "blur(120px)" }} />
          <div style={{ position: "absolute", bottom: "-20%", left: "10%", width: "500px", height: "500px", borderRadius: "50%", background: "#6B3FA0", opacity: 0.05, filter: "blur(100px)" }} />
        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-10">
                  ...
                  <div>
                    <motion.div initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.08 } } }}>
                      ...
                      >
                        Your facility hosts real buyers
                      ...
                      >
                        Prove the value. Justify the investment. Optimize every visitor experience.
                      </motion.p>
                      <motion.p
                        variants={fadeUp}
                        style={{
                      ...
                      >
                        Use our free Facilities ROX Calculator to measure the true return on your facility investments. Go beyond visitor counts and score what actually drives results:
                      </motion.p>
                      ...
                      <motion.div variants={fadeUp} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "48px" }}>
                        {[
                          { icon: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75", label: "Visitors captured vs. total tours" },
                          { icon: "M13 2L3 14h9l-1 8 10-12h-9l1-8", label: "Engagement depth by zone and content" },
                          { icon: "M18 20V10M12 20V4M6 20v-6", label: "Pipeline attribution from facility visits" },
                          ...
                          <div key={item.label} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                            ...
                            >
                              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#B8A0D8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                <path d={item.icon} />
                              </svg>
                            </div>
                            <span
                    ...
                    {/* Category 1 */}
                    <CategoryCard num="01" name="Visitor Capture Rate" weight="Worth 25% of your total score" score={categoryScores[0]}>
                      {renderField("Total Facility Tours This Quarter", "Estimated total number of facility tours your team gave this quarter.", c1_attendees, setC1Attendees, "e.g. 150")}
                      {renderField("Tours with Documented Interactions", "Total tours where a meaningful interaction was captured and documented.", c1_captured, setC1Captured, "e.g. 30")}
                    ...
                    <CategoryCard num="02" name="Engagement Quality" weight="Worth 25% of your total score" score={categoryScores[1]}>
                      {/* Toggle */}
                      <div className="flex items-center gap-1 mb-6" style={{ background: "rgba(107,63,160,0.06)", borderRadius: "24px", padding: "3px", width: "fit-content" }}>
                        <button
                          type="button"
                          ...
                          style={{
                            ...
                            borderRadius: "20px",
                            border: "none",
                            background: !engagementOptionA ? "#6B3FA0" : "transparent",
                      ...
                      {engagementOptionA ? (
                        <>
                          {renderField("Average Engagement Time Per Visit", "Average minutes visitors spend actively engaged during a tour.", c2a_avgTime, setC2aAvgTime, "e.g. 3")}
                          {renderField("Target Engagement Time", "Your target engagement time per visitor.", c2a_targetTime, setC2aTargetTime, "e.g. 5")}
                        </>
                      ) : (
                        <>
                          {renderField("Visitors Who Engaged Deeply", "Visitors who spent meaningful time engaging with content or demos.", c2b_meaningful, setC2bMeaningful, "e.g. 20")}
                          {renderField("Total Documented Interactions", "Same number from Category 1. Auto-populated if already entered above.", c2b_totalCaptured, setC2bTotalCaptured, "e.g. 30")}
                        </>
                      )}
                    </CategoryCard>
                    ...
                    <CategoryCard num="03" name="Follow-Up Speed" weight="Worth 25% of your total score" score={categoryScores[2]}>
                      {renderField(
                        "Average Days from Tour to CRM Entry",
                    ...
                    {/* Category 4 */}
                    <CategoryCard num="04" name="Pipeline Attribution" weight="Worth 25% of your total score" score={categoryScores[3]}>
                      {renderField("Tours That Advanced to Next Stage", "How many of your documented tours resulted in a qualified opportunity? A qualified opportunity had a meaningful interaction and fits your target profile.", c4_conversions, setC4Conversions, "e.g. 15")}
                      {renderField("Total Documented Interactions", "Same number from Category 1. Auto-populated if already entered above.", c4_totalCaptured, setC4TotalCaptured, "e.g. 30")}
                      {categoryScores[3] !== null && (
                        <div className="mt-2">
                          ...
                          <p style={{ fontFamily: "var(--font-inter)", fontWeight: 400, fontSize: "11px", color: "rgba(6,19,65,0.35)", marginTop: "2px" }}>
                            Industry average: ~10-20% tour-to-pipeline progression
                          </p>
                        </div>
                      )}
                    </CategoryCard>
                    ...
                    >
                      ...
                      {renderField("Estimated Value Per Opportunity", "What is the average deal value associated with a qualified opportunity from a facility tour? Use your best estimate if exact figures are unavailable.", bonus_valuePerLead, setBonusValuePerLead, "e.g. 25000", "$")}
